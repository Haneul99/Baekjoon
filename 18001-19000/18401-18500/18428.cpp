	...
	for (pair<int, int> t : tea) {
		for (int i = 0; i < 4; i++) {
			...
			while (1) {
				nx += dx[i];
				ny += dy[i];
				if (nx < 0 || nx > n - 1 || ny < 0 || ny > n - 1 || map[nx][ny] == 'O') break;
				if (map[nx][ny] == 'S') return 0;
			}
		}
	} 
	return 1;
...
int main() {
	...
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < n; j++) {
			scanf(" %c", &map[i][j]);
			if (map[i][j] == 'X') blank.push_back({ i, j });
			else if (map[i][j] == 'T') tea.push_back({ i, j });
		}
	}
...
장애물 세개 선택.
teacher에서 상하좌우로 이동하며 탐지할 수 있는 부분 계속해서 탐색
한 가지 경우라도 가능할 경우 ans는 1 이상, 어떤 경우에도 불가능하면 0
*/                      