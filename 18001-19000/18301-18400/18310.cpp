...
int main() {
	...
	long long n, ans, minDistance = LONG_MAX; cin >> n;
	vector<long long> houses, dist;
	for (int i = 0; i < n; i++) {
		long long x; cin >> x;
		houses.push_back(x);
	}
	...
	for (int i = 0; i < n - 1; i++) {
		dist.push_back(houses[i + 1] - houses[i]);
		distance += (n - 1 - i)*dist[i];
	}

	//안테나 설치 집
	ans = houses[0];
	for (int i = 1; i < n - 1; i++) {
		distance = distance - dist[i - 1] * (n - i) + dist[i - 1] * i;
