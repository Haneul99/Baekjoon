...
int main() {
	...
	long long ans = max * m;
	long long left = 1, mid, right = max * m;

	while (left <= right) {
		long long pp = 0;//present people
		mid = (left + right) / 2;
		for (int i = 0; i < n; i++) {
			pp += mid / time[i];
		}
...
이분탐색 문제.
문제에서 요구하는 것 = 최소 시간
즉, 이분탐색에서 구하고자하는 값(mid)은 시간임.

해당 시간(mid)에서 각 데스크가 처리할 수 있는 사람의 수를 모두 더해서
조건을 만족할 때의 최소 시간을 구함.

모든 값을 long long으로 했을 때 맞음.
*/                      